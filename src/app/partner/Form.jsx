...
const Form = () => {
  ...
  return (
    <div className='mb-[100px] mt-6  max-w-[1200px] mx-auto'>
      <h2 className=' text-[20px] sm:text-[30px] font-semibold mb-4 pl-5'>Want to partner with us</h2>
...
{field.type === 'radio' && field.name === 'other_notes' && (
          ...
          render={({ field: { onChange, value } }) => (
            <div className="flex flex-col">
              <Radio
                ...
                onChange={(e) => {
                  onChange(e);
                  if (e.target.value === "Industry Friends") {
                    setValue('industry_friend_detail', ''); // Initialize dynamic field
                  } else {
                    setValue('industry_friend_detail', undefined); // Clear field if other options selected
                  }
                }}
              />
              {option === "Industry Friends" && value === "Industry Friends" && (
                <div className="mt-2 ml-6"> {/* Indent to align with radio button */}
                  <label className="block text-black text-sm font-bold mb-2">
                    Others
                  </label>
                  <input
                    type="text"
                    {...register('industry_friend_detail', {
                      required: "Please specify your Industry Friend",
                    })}
                    placeholder="Reference name"
                    className={`block sm:w-[50%] w-full px-3 py-2 text-base border ${
                      errors.industry_friend_detail ? 'border-red-500' : 'border-gray-300'
                    } rounded-lg focus:outline-none`}
                  />
                  {errors.industry_friend_detail && (
                    <p className="text-red-500 text-sm mt-1">
                      {errors.industry_friend_detail.message}
                    </p>
                  )}
                </div>
              )}
  ...
  <button
    type="submit"
    className='bg-[#72B5EC] text-white font-semibold text-[16px] px-[50px] py-[15px] w-full sm:w-[70%] rounded-lg flex justify-center'
  >
    {isSubmitting ? (
      <svg
        aria-hidden="true"
